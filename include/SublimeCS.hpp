...
public:
    /**
     * Construct the full SublimeCS data structure, with support of auto-tuning
     * VALE. In our code, we follow the traditional naming convention of
     * calling arrays "rows" and each counter in each array a "column." 
     *
     ...
     */
    SublimeCS(size_t init_col_count, size_t init_row_count,
...
private:
    ...
    static constexpr float max_tail_probability = 0.01, retune_tail_frac = 0.03;
    ...
    // The value `n`, i.e., the total key count, has to reach to trigger a contraction.
    ...
    // Signs of the keys being operater over in the prefetch queue.
     ...
     */
    inline void push_prefetch_request(Sketch *sketch, const uint32_t pos,
                                      const bool sign, const OpType op);
     ...
     * This function recovers the counter's absolute value by fetching its stub
     * from its chunk, computing the value of the counter's extension (or tail)
     * if it is overflowing, and concatenating them. Finally, it recovers the
     * counter's sign from its sign bit within its stub, adjusting the result.
     *
     ...
     * This function sets the counter's stub as well as its sign bit, and
     * creates an extension for it or modifies it as necessary.
     *
    ...
    /**
     * Increments or decrements the counter at the offset `pos` within the
     * entire sketch when `inc=1` and `inc=0`, respectively. This entails
     * either 1) incrementing the counter's absolute value by incrementing its
     * stub and propagating a carry to its extension, or 2) decrementing the
     * counter's stub and borrowing from its extension. This function also
     * adjusts the sign bit of the counter accordingly.
     *
     * @param sketch The state of the sketch the counter must be modified in.
     * Determines the parameters of VALE.
     * @param pos The offset of the counter within the entire sketch.
     * @param inc Whether the counter should be incremented (`inc`=1) or
     * decremented (`inc`=0).
     */
     ...
     * increments or decrements the counter at the offset
     * `inter_cache_line_ind` when `inc=1` and `inc=0`, respectively. This
     * entails either 1) incrementing the counter's absolute value by
     * incrementing its stub and propagating a carry to its extension, or 2)
     * decrementing the counter's stub and borrowing from its extension. This
     * function also adjusts the sign bit of the counter accordingly.
     *
     * @param sketch The state of the sketch the counter must be modified in.
     * Determines the parameters of VALE.
     ...
     * decremented (`inc`=0).
     */
    inline void incdec_counter(Sketch *sketch, 
