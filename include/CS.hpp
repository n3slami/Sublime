...
public:
    CS(size_t init_col_count, size_t init_row_count, std::function<uint64_t(double)> expansion_f, uint32_t seed_gen_seed,
    ...
    void Insert(const char *elem, const uint32_t length) {
        ...
        if (using_tof_hashing) {
            ...
            for (int i = 0; i < row_count; i++) {
                ...
                sketch[pos] += (hash_value & 1ULL) ? 1 : -1;
                hash_value >>= 1;
            }
        }
        else {
    ...
    T Query(const char *elem, const uint32_t length) const {
        ...
        if (using_tof_hashing) {
            ...
            for (int i = 0; i < row_count; i++) {
                ...
                pos = pos < counter_count ? pos : pos - counter_count;
                hash_value >>= bias_range;
                res[i] = (hash_value & 1ULL) ? sketch[pos] : -sketch[pos];
                hash_value >>= 1;
            }
        }
        else {
            ...
            for (int i = 0; i < row_count; i++) {
                const uint32_t pos = col_count * i + hash_key(elem, length, i);
                res[i] = (sign_hash & 1ULL) ? sketch[pos] : -sketch[pos];
                sign_hash >>= 1;
            }
        }
