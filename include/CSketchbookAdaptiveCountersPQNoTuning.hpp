...
public:
    ...
    size_t Size(bool include_all_sketches = false) const {
        ...
        for (int32_t sketch_ind = sketches.size() - 1; sketch_ind >= 0; sketch_ind--) {
            ...
            res += base_size + extra_arrays * sizeof(uint32_t) * counter_per_cache_line;
...
private:
    ...
    __attribute__((always_inline))
    inline void push_prefetch_request(uint8_t *sketch, const uint32_t pos, const bool sign, const OpType op) {
        if constexpr (using_tof_hashing) {
    ...
    inline uint32_t get_extension_length(uint64_t extensions[]) const {
        if (num_extension_words == 1)
            return highbit_pos(extensions[0]) + 1;
        else if (num_extension_words == 2) {
            ...
            for (int32_t i = num_extension_words - 1; i >= 0; i--)
    ...
    inline int64_t get_counter(const uint8_t *sketch, const uint32_t pos) const {
        ...
        // Calculate the stub
        int64_t res;
        memcpy(&res, cache_line_ptr + word_update_byte_offset[inter_cache_line_ind], sizeof(res));
        const int64_t sign = (res >> (word_update_shamt[inter_cache_line_ind] + base_counter_size - 1)) & 1ULL;
        res = (res >> word_update_shamt[inter_cache_line_ind]) & (base_counter_size >> 1);
        ...
        if (has_extension) {
            ...
            extension_bitmap[num_extension_words - 1] = words[cache_line_size_words - num_extension_words] 
                                                                >> (64 - last_extension_word_bit_count);
            // Check for pointers
    ...
    inline void set_counter(uint8_t *sketch, const uint32_t pos, const int64_t _value) {
        ...
        // Set the stub
        uint64_t stamp;
        memcpy(&stamp, cache_line_ptr + word_update_byte_offset[inter_cache_line_ind], sizeof(stamp));
        stamp &= ~(BITMASK(base_counter_size) << word_update_shamt[inter_cache_line_ind]);
        stamp |= ((sign << (base_counter_size - 1)) | (value & BITMASK(base_counter_size - 1))) << word_update_shamt[inter_cache_line_ind];
        memcpy(cache_line_ptr + word_update_byte_offset[inter_cache_line_ind], &stamp, sizeof(stamp));
        ...
        if (update_state) {
            ...
            extension_bitmap[num_extension_words - 1] = words[cache_line_size_words - num_extension_words] 
                                                                >> (64 - last_extension_word_bit_count);
            const bool already_has_array_ptr = extension_bitmap[num_extension_words - 1] 
                                                                >> (last_extension_word_bit_count - 1);
    ...
    inline void incdec_counter(uint8_t *sketch, const uint32_t cache_line_ind, const uint32_t inter_cache_line_ind,
        ...
        const uint64_t stub_mask = BITMASK(base_counter_size - 1) << word_update_shamt[inter_cache_line_ind];
