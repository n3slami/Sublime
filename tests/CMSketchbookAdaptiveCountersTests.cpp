...
class CMSketchbookAdaptiveCountersTest {
public:
    static void CounterRW1() {
        auto f = [](size_t x) { return static_cast<uint64_t>(x * x); };
        CMSketchbookAdaptiveCounters sketch(10, 10, f, 1);
...
private:
    static void PrintSketch(CMSketchbookAdaptiveCounters &sketchbook) {
        uint8_t *sketch = sketchbook.sketches.back();
        const uint32_t cache_line_count = (sketchbook.row_count * sketchbook.col_count 
                                           + CMSketchbookAdaptiveCounters::counter_per_cache_line - 1)
                                          / CMSketchbookAdaptiveCounters::counter_per_cache_line;
        const uint32_t sep_1 = CMSketchbookAdaptiveCounters::counter_per_cache_line;
        const uint32_t sep_2 = sep_1 + CMSketchbookAdaptiveCounters::counter_per_cache_line 
                                        * CMSketchbookAdaptiveCounters::base_counter_size;
        const uint32_t sep_3 = sep_2 + CMSketchbookAdaptiveCounters::second_word_bit_count;
        std::cerr << "cache_line_count=" << cache_line_count << std::endl;
        for (int i = 0; i < cache_line_count; i++) {
            uint32_t cnt = 0;
            uint8_t *ptr = sketch + i * CMSketchbookAdaptiveCounters::cache_line_size_bytes;
            for (int j = 0; j < CMSketchbookAdaptiveCounters::cache_line_size_bytes; j++)
                for (int k = 0; k < 8; k++) {
                    if (cnt == sep_1)
                        std::cerr << " --- ";
                    else if (cnt == sep_2) {
                        ...
                        std::cerr << ' ';
                            ...
                            && (cnt - sep_1) % CMSketchbookAdaptiveCounters::base_counter_size == 0) {
                        std::cerr << ',';
                    }
                    std::cerr << ((ptr[j] >> k) & 1);
                    cnt++;
            ...
            if (words[CMSketchbookAdaptiveCounters::cache_line_size_words - 2] >> 63) {
                std::cerr << " ==== ";
                uint32_t *inner_ptr = reinterpret_cast<uint32_t *>(words[CMSketchbookAdaptiveCounters::cache_line_size_words - 1]);
                for (int j = 0; j < CMSketchbookAdaptiveCounters::counter_per_cache_line; j++)
...
TEST_SUITE("CMSketchbookAdaptiveCounters") {
    TEST_CASE("counter read/write") {
        CMSketchbookAdaptiveCountersTest::CounterRW1();
        CMSketchbookAdaptiveCountersTest::CounterRW2();
    }

    TEST_CASE("counter increment") {
        CMSketchbookAdaptiveCountersTest::CounterIncrement1();
        CMSketchbookAdaptiveCountersTest::CounterIncrement2();
    }

    TEST_CASE("counter decrement") {
        CMSketchbookAdaptiveCountersTest::CounterDecrement();
    }
}
