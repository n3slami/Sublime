...
inline CSketch *init_sketch(const uint32_t memory_budget, const uint32_t row_count) {
    const uint32_t counter_count = memory_budget;
    const uint32_t col_count = (counter_count + row_count - 1) / row_count;
    CSketch *sketch = new CSketch(col_count, row_count);
    sketch_w = col_count;
    sketch_d = row_count;
    return sketch;
}

inline void insert_sketch(CSketch *sketch, const std::string& key) {
    sketch->dynamic_sead_insert(key.c_str(), 1, gamma_2);
}

template<typename T>
inline void insert_sketch(CSketch *sketch, T key) {
...
template<typename T>
inline void delete_sketch(CSketch *sketch, const std::string& key) {
    sketch->dynamic_sead_insert(key.c_str(), -1, gamma_2);
}

template<typename T>
inline void delete_sketch(CSketch *sketch, T key) {
    char key_str[sizeof(key) + 1];
    memcpy(key_str, &key, sizeof(key));
    key_str[sizeof(key)] = 0;
    sketch->dynamic_sead_insert(key_str, -1, gamma_2);
...
inline int32_t query_sketch(CSketch *sketch, const std::string& key) {
    return sketch->dynamic_sead_query(key.c_str(), gamma_2);
}

template<typename T>
inline int32_t query_sketch(CSketch *sketch, T key) {
    char key_str[sizeof(key) + 1];
    memcpy(key_str, &key, sizeof(key));
    key_str[sizeof(key)] = 0;
...
inline uint32_t size_of_sketch(CSketch *sketch) {
    return sketch_w * sketch_d * sizeof(sead_c);
}


int main(int argc, char const *argv[]) {
    auto parser = init_parser("bench-SEADC");
