...
inline SublimeCS<true> *init_sketch(const uint32_t memory_budget, const uint32_t row_count,
                                    std::function<uint64_t(double)> f) {
    const uint32_t counter_count = force_counter_count > 0 ? force_counter_count 
                                                           : memory_budget / (static_cast<float>(SublimeCS<true>::cache_line_size_bytes) 
                                                                           / SublimeCS<true>::default_counter_per_cache_line);
    ...
    SublimeCS<true> *sketch = new SublimeCS<true>(col_count, row_count, f, seed);
    return sketch;
}

inline void insert_sketch(SublimeCS<true> *sketch, const std::string &key) {
    sketch->Insert(key.c_str(), key.size());
}

template<typename T>
inline void insert_sketch(SublimeCS<true> *sketch, T key) {
    sketch->Insert(key);
}

inline void delete_sketch(SublimeCS<true> *sketch, const std::string& key) {
    include_all_sketches = true;
    sketch->Delete(key.c_str(), key.size());
}

template<typename T>
inline void delete_sketch(SublimeCS<true> *sketch, T key) {
    include_all_sketches = true;
    sketch->Delete(key);
}

inline int32_t query_sketch(SublimeCS<true> *sketch, const std::string& key) {
    return sketch->Query(key.c_str(), key.size());
}

template<typename T>
inline int32_t query_sketch(SublimeCS<true> *sketch, T key) {
    return sketch->Query(key);
}

inline uint32_t size_of_sketch(SublimeCS<true> *sketch) {
    return sketch->Size(include_all_sketches);
}

inline std::unordered_map<std::string, uint32_t> get_extra_parameters(SublimeCS<true> *sketch) {
    ...
    res["total_adaptation_time"] = total_adaptation_time_cs;
    res["total_expansion_time"] = total_expansion_time_cs;
    res["total_contraction_time"] = total_contraction_time_cs;
    return res;
}


int main(int argc, char const *argv[]) {
    auto parser = init_parser("bench-SublimeCS-l2");
