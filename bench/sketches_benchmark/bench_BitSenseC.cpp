...
typedef OmniSketch::Sketch::BSCountSketch<key_len, no_layer, int32_t, OmniSketch::Hash::AwareHash> BSCSketchSpec;

inline BSCSketchSpec *init_sketch(const uint32_t memory_budget, const uint32_t row_count) {
    const uint32_t counter_count = memory_budget;
    const uint32_t col_count = (counter_count + row_count - 1) / row_count;
    constexpr double cnt_no_ratio = 0.14356297;
    std::vector<size_t> width_cnt = {4, 15};
    std::vector<size_t> no_hash = {3};
    BSCSketchSpec *sketch = new BSCSketchSpec(col_count, row_count,
                                              cnt_no_ratio, width_cnt, no_hash);
    return sketch;
}

inline void insert_sketch(BSCSketchSpec *sketch, const std::string& key) {
...
inline int32_t query_sketch(BSCSketchSpec *sketch, const std::string& key) {
    OmniSketch::FlowKey<key_len> flow_key(reinterpret_cast<const int8_t *>(key.c_str()));
...
inline uint32_t size_of_sketch(BSCSketchSpec *sketch) {
    return sketch->size();
}


int main(int argc, char const *argv[]) {
    auto parser = init_parser("bench-BitSenseC");
