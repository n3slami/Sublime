...
inline StingyC *init_sketch(const uint32_t memory_budget, const uint32_t row_count) {
    ...
    StingyC *sketch = new StingyC(col_count * row_count, row_count, seed);
    return sketch;
}
...
template<typename T>
inline void insert_sketch(StingyC *sketch, T key) {
    sketch->Insert(reinterpret_cast<char *>(reinterpret_cast<char *>(&key)), sizeof(key));
}

template<typename T>
inline void delete_sketch(StingyC *sketch, const std::string& key) {
    throw std::runtime_error("Deletes not implemented");
}

template<typename T>
inline void delete_sketch(StingyC *sketch, T key) {
    throw std::runtime_error("Deletes not implemented");
}

inline int32_t query_sketch(StingyC *sketch, const std::string& key) {
    return sketch->Query(key.c_str(), key.size());
}

template<typename T>
inline int32_t query_sketch(StingyC *sketch, T key) {
    return sketch->Query(reinterpret_cast<char *>(reinterpret_cast<char *>(&key)), sizeof(key));
}
